        ...
        <div class="grid grid-cols 2 gap-4">
            ...
            <form action="{{ $route }}" method="post" enctype="multipart/form-data"
                ...
                <input type="hidden" name="id"
                    value="@if (!empty($local->id)) {{ $local->id }}@elseif (!empty(old('id'))){{ old('id') }}@else{{ '' }} @endif">
                ...
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($local->nome)) {{ $local->nome }} @elseif(!empty(old('nome'))) {{ old('nome') }} @else {{ '' }} @endif">
                </label><br>

                <label class="block">
                    <span class="text-rose-700 font-semibold">Tipo de Local</span>
                    <select name="tipo_local_id"
                ...
                <label class="block">
                    ...
                    <input type="number" name="capacidade"
                        class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700"
                    value="@if(!empty($local->capacidade)){{$local->capacidade}}@elseif(!empty(old('capacidade'))){{old('capacidade')}}@else{{''}}@endif">
                </label><br>
                ...
                <label class="block">
                    ...
                    <input type="text" name="website"
                        class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($local->website)) {{ $local->website }} @elseif(!empty(old('website'))) {{ old('website') }} @else {{ '' }} @endif">
                </label><br>
                ...
                <label class="block">
                    ...
                    <input type="text" name="telefone"
                        class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($local->telefone)) {{ $local->telefone }} @elseif(!empty(old('telefone'))) {{ old('telefone') }} @else {{ '' }} @endif">
                </label><br>
                ...
                <label class="block">
                    ...
                    <input type="text" name="endereco"
                        class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($local->endereco)) {{ $local->endereco }} @elseif(!empty(old('endereco'))) {{ old('endereco') }} @else {{ '' }} @endif">
                </label><br>
                ...
                <label class="block">
                    ...
                    <input type="text" name="descricao"
                        class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($local->descricao)) {{ $local->descricao }} @elseif(!empty(old('descricao'))) {{ old('descricao') }} @else {{ '' }} @endif">
                </label><br>
            ...
            <button><a class="bg-rose-700 bg-opacity-20 text-rose-700 hover:text-white border border-rose-700 hover:bg-rose-800 focus:ring-4 focus:outline-none focus:ring-rose-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:border-rose-500 dark:text-rose-500 dark:hover:text-white dark:hover:bg-rose-600 dark:focus:ring-rose-900 w-1/3"
                        href="{{ route('local.index') }}"><i class="fa-solid fa-arrow-left"></i> Voltar</a></button>
        </div>
    </div>
    <br><br>
@endsection
