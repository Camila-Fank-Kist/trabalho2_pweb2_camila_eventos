        ...
        <div class="grid grid-cols 2 gap-4">
            ...
            <form action="{{ $route }}" method="post" enctype="multipart/form-data"
                ...
                <input type="hidden" name="id"
                    value="@if (!empty($evento->id)) {{ $evento->id }}@elseif (!empty(old('id'))){{ old('id') }}@else{{ '' }} @endif">
                ...
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($evento->nome)) {{ $evento->nome }} @elseif(!empty(old('nome'))) {{ old('nome') }} @else {{ '' }} @endif">
                </label><br>
                ...
                <div class="block grid grid-cols-2 gap-2">
                    <div>
                        <label class="block">
                            ...
                            class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-30 text-gray-600
                focus:ring-0 focus:border-rose-700"
                                value="@if(!empty($aluno->hora_inicio)){{ $aluno->hora_inicio }}@elseif(!empty(old('hora_inicio'))){{ old('hora_inicio') }}@else{{ '' }}@endif">
                        </label>
                    </div>
                    <div>
                        <label class="block">
                            ...
                            class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-30 text-gray-600
                focus:ring-0 focus:border-rose-700"
                                value=" @if(!empty($aluno->hora_fim)){{ $aluno->hora_fim }}@elseif(!empty(old('hora_fim'))){{ old('hora_fim') }}@else{{ '' }}@endif"><br><br>
                        </label>
                    </div>
                </div>
                ...
                <label class="block">
                    ...
                    <input type="text" name="descricao"
                        class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($evento->descricao)) {{ $evento->descricao }} @elseif(!empty(old('descricao'))) {{ old('descricao') }} @else {{ '' }} @endif">
                </label><br>
                ...
                <label class="block">
                    ...
                    <input type="text" name="precoBRL"
                        class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700"
                        value="@if (!empty($evento->precoBRL)) {{ $evento->precoBRL }} @elseif(!empty(old('precoBRL'))) {{ old('precoBRL') }} @else {{ '' }} @endif">
                </label><br>
                ...
                <img class="h-40 w-40 object-cover"
                    src="/storage/{{ $nome_imagem }}" width="300px"
                    alt="imagem"><br>
                    <input
            ...
            <button><a class="bg-rose-700 bg-opacity-20 text-rose-700 hover:text-white border border-rose-700 hover:bg-rose-800 focus:ring-4 focus:outline-none focus:ring-rose-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:border-rose-500 dark:text-rose-500 dark:hover:text-white dark:hover:bg-rose-600 dark:focus:ring-rose-900 w-1/3"
                        href="{{ route('evento.index') }}"><i class="fa-solid fa-arrow-left"></i> Voltar</a></button>
        </div>
    </div>
    <br><br>
@endsection
