...
<div class="mx-auto divide-y md:max-w-4xl">
    <div class="grid grid-cols 2 gap-4">
        ...
        <form action="{{ $route }}" method="post" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-6 mb-4 shadow-rose-700">
            ...
            <input type="hidden" name="id" value="@if (!empty($avaliacao->id)) {{ $avaliacao->id }}@elseif (!empty(old('id'))){{ old('id') }}@else{{ '' }} @endif">

            <input type="hidden" name="user_id" value="{{Auth::user()->id}}">

            <label class="block">
                <span class="text-rose-700 font-semibold">Evento</span>
                <select name="evento_id" class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-30 text-gray-600
            ...
            <label class="block">
                ...
                <input type="number" name="nota" class="mt-0 block w-full px-0.5 border-0 border-b-2 border-rose-700 border-opacity-40 text-gray-600
                focus:ring-0 focus:border-rose-700" value="@if(!empty($avaliacao->nota)){{ $avaliacao->nota }}@elseif(!empty(old('nota'))){{ old('nota') }}@else{{ '' }}@endif">
            </label><br>
            ...
            <label class="block">
                ...
                focus:ring-0 focus:border-rose-700" value="@if (!empty($avaliacao->descricao)) {{ $avaliacao->descricao }} @elseif(!empty(old('descricao'))) {{ old('descricao') }} @else {{ '' }} @endif">
            </label><br>
